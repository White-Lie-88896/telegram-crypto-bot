...
- **多数据源**：Cryptocompare（Binance）+ CoinGecko + Binance官方API
- **高可用性**：确保价格查询始终可用
- **智能优化**：记住当前可用的API，提升响应速度
- **30秒缓存**：减少API调用次数，避免速率限制
- **来源显示**：所有消息都会显示数据来自哪个API
...
#### 🔄 检查频率
- 监控引擎每 **30 秒** 检查一次所有活跃任务
- 快速响应价格变化的同时避免过度API调用
...
**A:** 编辑 `.env` 文件：
```env
CHECK_INTERVAL=30    # 修改为你想要的秒数（建议>=30避免API速率限制）
```
